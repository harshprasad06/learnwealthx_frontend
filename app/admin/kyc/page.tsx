    ...
    <div className="min-h-screen bg-gray-50 dark:bg-ink-950 transition-colors">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6 flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-ink-50">KYC Management</h1>
            <p className="text-gray-600 dark:text-ink-300 mt-1">Review and manage affiliate KYC submissions</p>
          </div>
          <Link
        ...
        {loading ? (
          <div className="text-center py-12 text-gray-600 dark:text-ink-300">Loading KYC submissions...</div>
        ) : (
          <>
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200 dark:divide-ink-800">
                  ...
                  <tbody className="bg-white dark:bg-ink-900 divide-y divide-gray-200 dark:divide-ink-800">
                    {kycs.map((kyc) => (
                      <tr key={kyc.id} className="hover:bg-gray-50 dark:hover:bg-ink-800 transition-colors">
                        <td className="px-6 py-4">
                          <div className="flex items-center space-x-3">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-gray-500 dark:text-ink-300">{kyc.affiliate.user.email}</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4 text-sm">
                          <div>
                            <p className="text-gray-900 dark:text-ink-50 capitalize">
                              {kyc.documentType?.replace('_', ' ') || 'N/A'}
                            </p>
                            <p className="text-gray-500 dark:text-ink-300 text-xs">{kyc.documentNumber || 'N/A'}</p>
                          </div>
                        </td>
                        <td className="px-6 py-4 text-sm">
                          <div>
                            <p className="text-gray-900 dark:text-ink-50">{kyc.accountHolderName || 'N/A'}</p>
                            <p className="text-gray-500 dark:text-ink-300 text-xs">
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center space-x-2">
                            <button
                              ...
                              className="px-3 py-1 text-xs bg-blue-100 dark:bg-mint-900/30 text-blue-700 dark:text-mint-400 rounded hover:bg-blue-200 dark:hover:bg-mint-900/50 transition-colors"
                            >
                              View
                            </button>
                            {kyc.status !== 'approved' && kyc.status !== 'rejected' && (
                              <>
                                <button
                                  ...
                                  disabled={processing}
                                  className="px-3 py-1 text-xs bg-green-100 dark:bg-green-950/50 text-green-700 dark:text-green-400 rounded hover:bg-green-200 dark:hover:bg-green-950/60 disabled:opacity-50 transition-colors"
                                >
                                  Approve
                                </button>
                                <button
                                  ...
                                  disabled={processing}
                                  className="px-3 py-1 text-xs bg-red-100 dark:bg-red-950/50 text-red-700 dark:text-red-400 rounded hover:bg-red-200 dark:hover:bg-red-950/60 disabled:opacity-50 transition-colors"
                                >
                                  Reject
                                </button>
                              </>
        ...
        {showModal && selectedKyc && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 dark:bg-black/70">
            <div className="bg-white dark:bg-ink-900 rounded-lg shadow-xl dark:shadow-black/40 max-w-3xl w-full mx-4 max-h-[90vh] overflow-y-auto transition-colors">
              <div className="p-6">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-2xl font-bold text-gray-900 dark:text-ink-50">KYC Details</h2>
                  <button
                    onClick={() => {
                      setShowModal(false);
                      setSelectedKyc(null);
                ...
                <div className="space-y-6">
                  {/* Affiliate Info */}
                  <div className="border-b border-gray-200 dark:border-ink-800 pb-4">
                    <h3 className="text-lg font-semibold mb-3 text-gray-900 dark:text-ink-50">Affiliate Information</h3>
                    <div className="flex items-center space-x-3">
                      {renderAvatar(
                      ...
                      <div>
                        <p className="font-medium text-gray-900 dark:text-ink-50">{selectedKyc.affiliate.user.name || 'No name'}</p>
                        <p className="text-sm text-gray-600 dark:text-ink-300">{selectedKyc.affiliate.user.email}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Document Info */}
                  <div className="border-b border-gray-200 dark:border-ink-800 pb-4">
                    <h3 className="text-lg font-semibold mb-3 text-gray-900 dark:text-ink-50">Identity Document</h3>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <p className="text-sm text-gray-500 dark:text-ink-300">Document Type</p>
                        <p className="font-medium text-gray-900 dark:text-ink-50 capitalize">
                          {selectedKyc.documentType?.replace('_', ' ') || 'N/A'}
                        </p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-500 dark:text-ink-300">Document Number</p>
                        <p className="font-medium text-gray-900 dark:text-ink-50">{selectedKyc.documentNumber || 'N/A'}</p>
                      </div>
                    </div>
                    <div className="mt-4 grid grid-cols-3 gap-4">
                      {selectedKyc.documentFront && (
                        <div>
                          <p className="text-sm text-gray-500 dark:text-ink-300 mb-2">Document Front</p>
                          <a
                            href={getDocumentUrl(selectedKyc.id, 'documentFront')}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="text-blue-600 dark:text-mint-400 hover:underline text-sm"
                          >
                            Open full document →
                          </a>
                        </div>
                      ...
                      {selectedKyc.documentBack && (
                        <div>
                          <p className="text-sm text-gray-500 dark:text-ink-300 mb-2">Document Back</p>
                          <a
                      ...
                      {selectedKyc.addressProof && (
                        <div>
                          <p className="text-sm text-gray-500 dark:text-ink-300 mb-2">Address Proof</p>
                          <a
                  ...
                  {/* Bank Details */}
                  <div className="border-b border-gray-200 dark:border-ink-800 pb-4">
                    <h3 className="text-lg font-semibold mb-3 text-gray-900 dark:text-ink-50">Bank Account Details</h3>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <p className="text-sm text-gray-500 dark:text-ink-300">Account Holder Name</p>
                        <p className="font-medium text-gray-900 dark:text-ink-50">{selectedKyc.accountHolderName || 'N/A'}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-500 dark:text-ink-300">Account Number</p>
                        <p className="font-medium text-gray-900 dark:text-ink-50">{selectedKyc.bankAccountNumber || 'N/A'}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-500 dark:text-ink-300">IFSC Code</p>
                        <p className="font-medium text-gray-900 dark:text-ink-50">{selectedKyc.bankIFSC || 'N/A'}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-500 dark:text-ink-300">Bank Name</p>
                        <p className="font-medium text-gray-900 dark:text-ink-50">{selectedKyc.bankName || 'N/A'}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Status & Dates */}
                  <div>
                    <h3 className="text-lg font-semibold mb-3 text-gray-900 dark:text-ink-50">Status Information</h3>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                      ...
                      <div>
                        <p className="text-sm text-gray-500 dark:text-ink-300">Submitted At</p>
                      ...
                      {selectedKyc.reviewedAt && (
                        <div>
                          <p className="text-sm text-gray-500 dark:text-ink-300">Reviewed At</p>
                          <p className="font-medium text-gray-900 dark:text-ink-50">
                      ...
                      {selectedKyc.rejectionReason && (
                        <div className="col-span-2">
                          <p className="text-sm text-gray-500 dark:text-ink-300">Rejection Reason</p>
                  ...
                  {selectedKyc.status !== 'approved' && selectedKyc.status !== 'rejected' && (
                    <div className="border-t border-gray-200 dark:border-ink-800 pt-4">
                      <h3 className="text-lg font-semibold mb-3 text-gray-900 dark:text-ink-50">Reject KYC</h3>
                      <div className="mb-4">
                        <label className="block text-sm font-medium text-gray-700 dark:text-ink-200 mb-2">
